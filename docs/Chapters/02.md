...
The `null` type represents an intentional absence of any value. Unlike `undefined`, which often arises from language mechanics, developers assign `null` explicitly to indicate "no value here."

**Common Pitfall:** The `typeof null === "object"` behavior is a famous JavaScript quirk that dates back to the language's early days. In TypeScript, you'll often need to handle this explicitly when checking for null values.
...
Sometimes you know more about a type than TypeScript can infer. Type assertions let you tell TypeScript "trust me, I know what this is":
...
The `|` operator creates union types, which can hold values of multiple types:
...
Literal types form the foundation of discriminated unions, a powerful pattern for modeling variant data structures:
...
Functions with `void` return type can return `undefined` explicitly, but the return value should be ignored.
...
`readonly` is a TypeScript-only feature that prevents modification of object properties or array elements at the type level:
...
TypeScript lets you create new string types using template syntax, enabling powerful pattern matching at the type level:
...
With `strictNullChecks` enabled (recommended), TypeScript treats `null` and `undefined` as distinct types:
