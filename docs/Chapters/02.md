...
TypeScript builds a sophisticated type system on top of JavaScript's runtime behavior.
This chapter covers foundational concepts you need to understand how TypeScript works, 
distinguishing between JavaScript features (available at runtime) and TypeScript features (compile-time only).
This distinction is crucial: JavaScript features exist when your code runs in the browser or Node.js, 
while TypeScript features help you catch errors and write better code during development but disappear after compilation.

Understanding these fundamentals prepares you for more advanced TypeScript features like interfaces, generics, and complex type manipulations.
The type system becomes more powerful as you combine these basic building blocks in sophisticated ways.

## JavaScript's Runtime Types

TypeScript's type system mirrors JavaScript's runtime types.
These are the actual data types that exist when your code executes, so you can inspect them using JavaScript's `typeof` operator.
...
`string`s represent textual data.
JavaScript strings are immutable and can contain any Unicode characters.

`number`s represent both integers and floating-point numbers.
JavaScript uses IEEE 754 double-precision for all numbers, which has limitations for very large integers.

`bigint`s represent arbitrarily large integers, beyond the safe integer limits of JavaScript's `number` type.
You create them by appending `n` to integer literals, like `123n`.
This solves the precision problems with very large numbers.

`symbol`s represent unique, opaque values that serve as property keys or internal identifiers.
...
JavaScript assigns `undefined` automatically in many situations.

A `null` represents an intentional absence of any value.
Unlike `undefined`, that often arises from language mechanics, developers assign `null` explicitly to indicate "no value here."

The `typeof null === "object"` behavior is a famous JavaScript quirk that dates back to the language's early days.
In TypeScript, you'll often need to handle this explicitly when checking for null values.

### Variable Declaration Keywords: `const` and `let`

Both `const` and `let` are JavaScript features introduced in ES2015 (ES6).
They exist at runtime and control how variables are bound and scoped.

`const` produces an immutable binding to a value:
...
`const` only prevents reassignment of the variable itself, not mutation of the value it holds:
...
`let` produces a mutable, block-scoped variable binding:
...
TypeScript adds compile-time type checking on top of JavaScript's runtime types.

### Type Annotations and Inference

In TypeScript you can explicitly annotate types or let the compiler infer them for you.
...
In most cases, TypeScript's inference is excellent, so you don't need to write type annotations everywhere.
However, understanding when and why to use explicit annotations is important.
...
**TypeScript at compile-time:** The `const` version has a more specific type for better type checking
...
The `number[]` syntax is preferred in modern TypeScript because it's more concise, reads naturally with array literals, 
and handles nested arrays more cleanly (`number[][]` vs `Array<Array<number>>`).
You'll sometimes see `Array<T>` in older codebases or when working with complex generic constraints.
...
If you're wrong, your code might break at runtime.

JSX (JavaScript XML) and TSX (TypeScript XML) are syntax extensions used in React that allow HTML-like code inside JavaScript/TypeScript.
In `.jsx` and `.tsx` files, the `<type>` assertion syntax conflicts with JSX element syntax (`<div>`, `<button>`, etc.), so always use the `as` syntax in React projects.

## Literal Types and Unions

TypeScript creates types representing exact values, not just general categories.
...
They create compile-time guarantees about exact values.
...
You can think of literal union types as a lightweight enumeration.
Both define a finite set of valid values, but enumeration introduce new named identifiers, while literal types work directly with the actual values.
For many cases where the values themselves carry meaning (like `"left"` or `"center"`), literal types are more idiomatic in TypeScript.

### Discriminated Unions

Literal types form the foundation of discriminated unions, 
a powerful pattern for modeling variant data structures (data that takes different forms but represents the same concept):
...
The `status` field acts as a discriminant that helps TypeScript narrow the type within control flow.
...
Avoid `any` because it defeats the purpose of using TypeScript.
Use it sparingly and deliberately, with a plan to replace it with proper types.

### `unknown` - The Safe Alternative

Like `any`, you can assign anything to `unknown`, but you must then narrow the type before using it:
...
Use `unknown` when you genuinely don't know what type something will be, but want to handle it safely.
`unknown` is much safer than `any` because it forces you to check types before using values.
...
`n => n * 2` is an arrow function (ES6 JavaScript feature), providing a concise way to write `function(n) { return n * 2; }`.
...
This creates the most specific possible type for a value, making it deeply immutable:
...
TypeScript's immutability features provide compile-time safety, not runtime immutability.
For actual runtime immutability, use `Object.freeze()`:
...
This pattern proves particularly powerful for modeling APIs, configuration keys, CSS class names, 
or any domain where string patterns need compile-time validation.

### Function Type Basics

We'll cover functions in detail later, but here are the fundamentals:
...
1. **JavaScript foundations:** Primitive types and `const` bindings provide the runtime behavior that TypeScript's types describe.

2. **TypeScript enhancements:** Type annotations, inference, and special types like `unknown` and `never` catch errors before runtime.

3. **Literal types and unions:** Enable precise modeling of allowed values and discriminated unions for variant data.

4. **Immutability tools:** `readonly`, `as const`, and proper use of `const` create safer, more predictable code.

5. **Template literal types:** Allow compile-time validation of string patterns, perfect for APIs and configuration.
