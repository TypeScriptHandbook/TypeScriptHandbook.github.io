...
In `.jsx` and `.tsx` files, the `<type>` assertion syntax conflicts with JSX element syntax (`<div>`, `<button>`, etc.), so always use the
...
** If you know enumerations (enums) from other languages, you can think of literal union types as a lightweight alternative.
...
For many cases where the values themselves carry meaning (like
