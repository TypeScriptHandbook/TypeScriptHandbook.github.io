...
Arrow functions provide concise syntax and lexical `this` binding. They work best for short functions and callbacks:
...
All parameters are required by default. Calling the function with missing arguments produces a compile-time error:
...
Use overloads when you need precise return type relationships. Use union types when the relationship is simpler.
